...
@app.route('/home')
def home():
    # Displays the first 5 posts on home page
...
def post(post_id):
    # Give post with post_id, or return 404 if it doesn't exist
...
def update_post(post_id):
    ...
    # Updates title and content if form is valid
    if form.validate_on_submit():
        post.title = form.title.data
        post.content = form.content.data
        db.session.commit() # No need to add, as info are already in db and we only update them
        flash('Post updated!', 'success')
        return redirect(url_for('post', post_id=post.id))
    elif request.method == 'GET':
        # Fills in the update post with current data
...
def reset_token(token):
    '''Resets user's password, with their token active'''
    # Make sure user is logged out before resetting password
    if current_user.is_authenticated:
        return redirect(url_for('home'))
